...
##			errorfile=self.blog.theme.error
		...
		try:
			content=silog_template.render(self.blog.theme,errorfile, self.template_vals)
		except TemplateDoesNotExist:
			try:
				content=silog_template.render(self.blog.theme,"error.html", self.template_vals)
			except TemplateDoesNotExist:
				content=silog_template.render(self.blog.default_theme,"error.html", self.template_vals)
			except:
				content=message
		except:
			content=message
	...
	def get_render(self,template_file,values):
		...
		try:
			...
			html = silog_template.render(self.blog.theme, template_file, self.template_vals)
		except TemplateDoesNotExist:
			#sfile=getattr(self.blog.default_theme, template_file)
			html = silog_template.render(self.blog.default_theme, template_file, self.template_vals)
