...
class Blog(db.Model):
	...
	title = db.StringProperty(multiline=False,default='Silog')
	subtitle = db.StringProperty(multiline=False,default='This is a Silog.')
	...
	def __init__(self,
			   ...
			   _from_entity=False,
			   **kwds):
		from silog_plugin import Plugins
...
def InitBlogData():
	...
	entry.content=_('<p>Welcome to silog. This is your first post. Edit or delete it, then start blogging!</p>').decode('utf8')
	entry.save(True)
	link=Link(href='http://www.softwareinlife.com',linktext=_("Sil's home").decode('utf8'))
	link.put()
	return g_blog
